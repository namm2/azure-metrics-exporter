...
```bash
go install github.com/namm2/azure-metrics-exporter
```

## Usage
```bash
./azure-metrics-exporter --help
```
...
* If using Workload Identity:
  * The [Microsoft Entra Workload ID](https://learn.microsoft.com/en-us/azure/aks/workload-identity-overview) must be configured and granted "Monitoring Reader".
  * `subscription_id`, and `workload_identity` will be needed in your credentials configuration.
...
If you're using Microsoft Entra Workload Identity, there's only `workload_identity: true` needed and you don't need to set `client_id`, `client_secret`, and `tenant_id`.
